...
C     Reset observed data
     ...
     +    EXP(Error*Sigma-Sigma**2.0/2.0)
        ...
        IF (MOD(Iyear,4).EQ.0) THEN
         Nhat2(Iyear) = Nhat(Iyear)
         ObsCV2(Iyear) = ObsCV(Iyear)
        ENDIF
2000  CONTINUE      
C
...
C     Find the abundance 
      ...
      SumY = 0
      SumXY = 0
      DO 30000 Iyear = TheYear,MAX(-50,FirstYrAbund),-1
       IF (ObsCV2(Iyear).GT.0.AND.
     +   (Nfound.LT.5.OR.TheYear-Iyear.LT.20)) THEN
        Nfound = Nfound + 1
        SumX = SumX + Iyear
        SumXY = SumXY + log(0.1d0+NHat2(Iyear))*FLOAT(Iyear)
        SumY = SumY + log(0.1d0+NHat2(Iyear))
        SumXX = SumXX + Iyear*Iyear
       ENDIF
30000 CONTINUE
      Slope = (SumXY-SumX*SumY/FLOAT(Nfound))/
     +        (SumXX-SumX*SumX/FLOAT(Nfound))
      XBAR = SumX/FLOAT(Nfound)
      Intercept = SumY/FLOAT(Nfound) - Slope*XBAR
      EChange2 = 1.0-EXP(Slope*16)
C
C     Sigma of beta
      SS = 0
      Denom = 0
      Nfound = 0
      DO 31000 Iyear = TheYear,MAX(-50,FirstYrAbund),-1
       IF (ObsCV2(Iyear).GT.0.AND.
     +   (Nfound.LT.5.OR.TheYear-Iyear.LT.20)) THEN
        Nfound = Nfound + 1
        Pred = Intercept + Slope*Iyear
        SS = SS + (Pred-log(0.1d0+NHat2(Iyear)))**2.0
        Denom = Denom + (Iyear-XBAR)**2.0
       ENDIF
31000 CONTINUE
      IF (Nfound.LE.2) THEN
      ...
      ENDIF
      VarChange = (16*EXP(Slope*16))**2.0*SigmaSlope**2.0
      CVSlope = ABS(SQRT(VarChange)/ABS(EChange2+0.0001))
C      WRITE(*,*) "C",CVSLOPE
C
C     Now simulate
      ISEED = -190101
      DO 40000 Isim = 1,NsimDCAC
...
C     Find the abundance 
     ...
     +   (Nfound.LT.5.OR.TheYear-Iyear.LT.20)) THEN
       ...
       ENDIF
31000 CONTINUE
      Slope2 = (SumXY-SumX*SumY/FLOAT(Nfound))/
     +        (SumXX-SumX*SumX/FLOAT(Nfound))
      XBAR = SumX/FLOAT(Nfound)
      Intercept = SumY/FLOAT(Nfound) - Slope2*XBAR
C
C     Sigma of beta
      SS = 0
      Denom = 0
      Nfound = 0
      DO 32000 Iyear = TheYear,-50,-1
       IF (ObsCV2(Iyear).GT.0.AND.
     +  (Nfound.LT.5.OR.TheYear-Iyear.LT.20)) THEN
        Nfound = Nfound + 1
        Pred = Intercept + Slope2*Iyear
        SS = SS + (Pred-log(0.1d0+NHat2(Iyear)))**2.0
        Denom = Denom + (Iyear-XBAR)**2.0
       ENDIF
32000 CONTINUE
